...
export default async function piTestOracle(pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		description: "Generate a minimal failing test (RED) for a given function signature. Returns the test file content and records the test in RED state.",
		parameters: {
			type: "object",
			properties: {
				signature: { type: "string", description: "The function signature to generate a test for" },
				importPath: { type: "string", description: "The module path to import the function from (e.g., '../src/auth')" },
				symbolName: { type: "string", description: "Optional: override the symbol name" },
				testFilePath: { type: "string", description: "Optional: override the test file path" },
			},
			required: ["signature", "importPath"],
		},
		handler: async (_ctx: any, args: any) => {
			try {
				const generated = generator.generateTest(args.signature, args.importPath, {
	...
	pi.registerTool({
		...
		description: "List all tracked tests and their TDD states (RED, GREEN, REFACTOR).",
		parameters: {
			type: "object",
			properties: {
				state: { type: "string", enum: ["RED", "GREEN", "REFACTOR", "UNKNOWN"] },
			},
		},
		handler: async (_ctx: any, args: any) => {
	...
	pi.registerTool({
		...
		description: "Find code coverage gaps below a threshold and recommend files that need tests.",
		parameters: {
			type: "object",
			properties: {
				threshold: { type: "number", description: "Coverage percentage threshold (default 80)" },
			},
		},
		handler: async (_ctx: any, args: any) => {
			...
			if (!reportPath) {
				return { error: "No coverage report found. Run `npx jest --coverage` first." };
			}
	...
	pi.registerTool({
		...
		description: "Read the latest pi-audit-master report and generate prioritized test suggestions for each finding.",
		parameters: { type: "object", properties: {} },
		handler: async (_ctx: any, _args: any) => {
			const findings = nexus.readLatestAuditReport();
			if (findings.length === 0) {
				return { message: "No audit findings found. Run pi-audit-master first.", suggestions: [] };
			}
	...
	if (typeof piAny.registerCommand === "function") {
		piAny.registerCommand("test-oracle", {
			description: "Test oracle commands: generate, status, coverage, from-audit",
			handler: async (args: any, ctx: any) => {
				const argStr = typeof args === "string" ? args.trim() : "";
				const [subcommand, ...rest] = argStr.split(/\s+/);
				...
				switch (subcommand?.toLowerCase()) {
					...
					case "coverage": {
						const reportPath = coverage.findCoverageReport();
						if (!reportPath) {
							ctx.ui.notify("No coverage report found. Run `npx jest --coverage` first.", "warning");
							return;
						}
						const gaps = reportPath.endsWith(".json")
							? coverage.parseJsonCoverage(reportPath)
					...
					default: {
						ctx.ui.notify(
							`pi-test-oracle commands: status, coverage, from-audit. Or use the test_oracle_generate tool.`,
							"info",
						);
					}
				}
	...
	console.log("[pi-test-oracle] Loaded. Tools: test_oracle_generate, test_oracle_status, test_oracle_coverage, test_oracle_from_audit");
}
